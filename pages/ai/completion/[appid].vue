      ...
      <Drawer v-model:open="isDrawerOpen" class="w-full sm:max-w-[90%]">
        <DrawerContent class="h-[90vh]">
          <DrawerHeader>
            ...
            <DrawerDescription v-if="generatedContent">
...
const { appInfo, isLoadingApp, generatedContent } = storeToRefs(difyChatStore);
...
// 设置动态标题
useHead(() => ({
  title: appInfo.value?.name || "Dify Completion",
}));
...
difyChatStore.setType("completion");
initAppId();
...
// 监听生成内容变化，自动打开抽屉
watch(generatedContent, (newValue) => {
